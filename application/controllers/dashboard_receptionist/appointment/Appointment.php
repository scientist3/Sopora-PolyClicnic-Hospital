...
class Appointment extends CI_Controller {
    ...
    public function create()
    {
        ...
        $data['appointment'] = (object)$postData = [
            ...
            'amount'         => $this->input->post('doctor_cons_fee',true), 
        ...
        //check appointment exists/ no exist= means =true
    ...
    */
    ...
    public function doctor_by_department()
    {
        ...
        if (!empty($department_id)) {
            $query = $this->db->select('user_id,firstname,lastname,consfee')
                ->from('user')
            ...
            if ($query->num_rows() > 0) {
                foreach ($query->result() as $doctor) {
                    $option .= "<option value=\"$doctor->user_id\">$doctor->firstname $doctor->lastname -Rs $doctor->consfee</option>";
                } 
                $data['message'] = $option;
                $data['status'] = true;
            } else {
    ...
    public function doctor_cons_fee()
    {
        $doctor_id = $this->input->post('doctor_id');
        //$data['message'] = $doctor_id;
        //$data['status'] = true;
        if (!empty($doctor_id)) {
            $data['message'] = "If";
            $data['status'] = true;

            $query = $this->db->select('consfee')
                ->from('user')
            ...
            if ($query->num_rows() > 0) {
                ...
                $feee=$query->row();
                $data['message'] = $feee->consfee;
                $data['status'] = true;
            }
            else{
                $data['message'] = "Fee Details Unavailable";
                $data['status'] = true;
            }
        }
        else
        {
            $data['message'] ='invalid_doctor_Id';
            $data['status'] = null;            
        }
        echo json_encode($data);
    }
